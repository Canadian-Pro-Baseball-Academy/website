...
export const CoachCardProto: React.FC<Coach & { index: number }> = ({
  name,
  headshot,
  role,
  index,
...
export const CoachCardMini: React.FC<Coach & { index: number }> = ({
  ...
  return (
    <div className="max-w-1/2 sm:max-w-1/4 group w-1/2 overflow-hidden p-4 sm:w-1/4">
      <div className="relative w-full overflow-hidden">
        <AspectRatio ratio={1}>
          {typeof headshot === "object" && headshot !== null && (
            <Media
              ...
              fill
              resource={headshot}
              className="pointer-events-none h-full w-full bg-muted saturate-0 transition-all duration-100  group-hover:saturate-100"
              style={{ objectFit: "contain", height: "100%", width: "100%" }}
            />
          )}
        </AspectRatio>
...
export const CoachCard: React.FC<Coach> = ({
  name,
  headshot,
  role,
  career,
  ...
  return (
    <div className="flex w-full flex-col rounded-lg bg-primary sm:flex-row">
      ...
      <div className="flex-1 space-y-4 px-6 py-8">
        <div className="text-background">
          <h1 className="scroll-m-20 border-b border-background/50 pb-1 text-3xl font-bold tracking-tight transition-colors first:mt-0">
            {name}
          </h1>
          <h2 className="mt-4 scroll-m-20 text-xl font-semibold tracking-tight text-accent">
            {role?.title}
          </h2>
          <div className="text">
        ...
        <div className="text-background">
          <h2 className="scroll-m-20 text-xl font-semibold tracking-tight text-accent">
            Career Achievements
          </h2>
          <div className="text">
            {career?.map((achievement) => <p>{achievement.achievement}</p>)}
