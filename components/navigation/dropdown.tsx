...
const HighlightItem: React.FC<HighlightItem> = ({ highlightLink }) => {
  return (
    <li className="row-span-3 col-span-4">
...
const LinkItem: React.FC<LinkItem> = ({ link }) => {
  return (
    <li className="block col-span-4 select-none space-y-1 rounded-md p-3 leading-none no-underline outline-none transition-colors hover:bg-muted focus:bg-muted">
      <NavigationMenuLink asChild>
        <PayloadLink {...link} />
      </NavigationMenuLink>
    </li>
...
export const MobileDropdown: React.FC<Header["mainMenu"]["items"][0]> = ({
  ...
  return (
    <AccordionItem value={label}>
      ...
      <AccordionContent>
        <ul className="grid grid-cols-4 sm:grid-cols-8 md:grid-cols-12 gap-1 p-4">
          {blocks.map((block, index) => {
            const { blockType } = block;
